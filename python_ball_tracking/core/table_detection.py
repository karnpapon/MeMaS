...
from cv2 import threshold, THRESH_BINARY, dilate, floodFill, circle, HoughLines, erode, rectangle, VideoWriter
from trace_header import videoFile, findIntersection, calculatePixels
from table_mapping import courtMap, showLines, showPoint, heightP, widthP, givePoint
...
clip = VideoWriter('resources/video_output2.mp4',
                   fourcc, 25.0, (widthP, heightP))
processedFrame = None
...
while video.isOpened():
  ...
  bw = threshold(gry, 50, 255, THRESH_BINARY)[1]
  canny = Canny(bw, 100, 200)

  # plt.subplot(121),plt.imshow(bw, cmap='gray')
  # plt.title('Original Image bw'), plt.xticks([]), plt.yticks([])
  ...
  hPLines = HoughLinesP(canny, 1, pi/180, threshold=50,
  ...
  for hPLine1 in hPLines:
    ...
    for hPLine2 in hPLines:
      ...
      intersect = findIntersection(
          Line1, Line2, Line1x1-200, Line1y1-200, Line1x2+200, Line1y2+200)
  ...
  # plt.subplot(121), plt.imshow(eroded)
  # plt.title('eroded Image'), plt.xticks([]), plt.yticks([])
  # plt.subplot(122), plt.imshow(cannyMain, cmap='gray')
  # plt.title('cannyMain Image'), plt.xticks([]), plt.yticks([])
  ...
  for hLine in hLines:
    for rho, theta in hLine:
      ...
      intersectxF = findIntersection(
          axis.bottom, [[x1, y1], [x2, y2]], -extraLen, 0, width+extraLen, height)
      intersectyO = findIntersection(
          axis.left, [[x1, y1], [x2, y2]], -extraLen, 0, width+extraLen, height)
      intersectxO = findIntersection(
          axis.top, [[x1, y1], [x2, y2]], -extraLen, 0, width+extraLen, height)
      intersectyF = findIntersection(
          axis.right, [[x1, y1], [x2, y2]], -extraLen, 0, width+extraLen, height)
...
# clip = VideoWriter('./overlay_ball_out.mp4',fourcc,25.0,(widthP,heightP))
# counter = 0
# video = VideoCapture('./video_output2.mp4')
# writeFlag = False
# while video.isOpened():
#     ret, frame = video.read()
#     if frame is None:
